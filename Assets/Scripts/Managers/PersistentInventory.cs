...
public static class PersistentInventory
{
    ...
    public struct DisplayItem
    {
        ...
        public DisplayItem(string id, string label, Color color, int count)
        {
            Id = id;
            Label = label;
            Color = color;
    ...
    {
        return Mathf.Max(0, PlayerPrefs.GetInt(Key(id), 0));
    }

    public static void Collect(string id)
    {
        PlayerPrefs.SetInt(Key(id), Count(id) + 1);
    ...
    private static void AddIfHeld(List<DisplayItem> items, string id, string label, Color color)
    {
        ...
        if (count > 0)
        {
            items.Add(new DisplayItem(id, label, color, count));
        }
    }
